...
/**
 * @typedef {Object} ShortkeyInitDeps
 * @property {object} ui
...
/**
 * Shortkey 도메인 모듈. 바닐라 Shortcuts에 defaultKeys·customKeys·convert·renderSettings·init·registerKeys를 붙인다.
 * `initialize()` 호출 전까지 단축키 정의 API는 없다.
 *
 * @returns {object & { initialize: (deps: ShortkeyInitDeps) => void }}
 */
const shortkeyModule = () => {
	...
	shortkey.initialize = ({ ui, sheet, video, i18n }) => {
		...
		const { init, registerKeys } = createInit({ shortkey, sheet, ui });
		shortkey.init = init;
