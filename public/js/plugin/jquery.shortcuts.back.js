...
(function($) {
	...
	var getMaskObject = function(mask) {
		...
		$.each(items, function(i, item) {
			if (item === 'ctrl' || item === 'alt' || item === 'shift') {
				...
				obj.which = $.Shortcuts.Code[item] || item.toUpperCase().charCodeAt();
			}
		});
	...
	var checkIsInput = function(target) {
		...
		return (name === 'input' && $.inArray(type, ['text', 'password', 'file', 'search', 'number', 'url']) > -1) || name === 'textarea';
	};
	...
	var run = function(type, e) {
		...
		$.each(shortcuts, function(i, shortcut) {
			...
			if (!isInput || shortcut.enableInInput) {
				if (!shortcut.isPrevented) {
					e.preventDefault();
					shortcut.isPrevented = true;
				}
	...
	$.Shortcuts.Code = {
		...
		']' : 221,
		'\'' : 222,
		'minus': $.browser.opera ? [109, 45] : $.browser.mozilla ? [109, 173, 189] : [189, 109],
		'plus': $.browser.opera ? [61, 43] : $.browser.mozilla ? [61, 107, 187] : [187, 107]
	};
	/**
	 * Start reacting to shortcuts in the specified list.
	 * @param {String} [list] List name
	 */
	$.Shortcuts.callback = function(callback){
		isCallback = callback;
	};
	$.Shortcuts.start = function(list) {
		...
		$(document).on(($.browser.opera ? 'keypress' : 'keydown') + '.shortcuts', function(e) {
	...
	$.Shortcuts.removeAll = function() {
		$.each(arrayParams, function(eq,params){
			$.Shortcuts.remove(params);
		});
		arrayParams = [];
	};
	$.Shortcuts.search = function(str) {
		var result = true;
		var isSpace = -1;
		$.each(arrayParams, function(eq,params){
			if (params.mask == str) {
